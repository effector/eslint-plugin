...
`useUnit` is a modern hook which allows you to bind stores, events, and effects to React render cycle.

`useUnit` should be preferred to deprecated hooks `useStore`/`useEvent` because it has several advantages:

- it enables store updates batching, which can significantly increase React performance
- it does not have name collision with React `useEvent` hook
